...
SLEEP=5
 until [ $RETRY -gt 3 ]
 do
    echo "Try $RETRY Uploading Certificates to $KONG_ADDR"
    RETRY=$((RETRY+1))
    DELETE_STATUS=$(curl --write-out "%{http_code}" --silent --output /dev/null -X DELETE "$KONG_ADDR"/certificates/"$DOMAIN")

    if [ "$DELETE_STATUS" -eq 404 ] || [ "$DELETE_STATUS" -eq 204 ] ; then
      HTTP_STATUS=$(curl --write-out "%{http_code}" --silent --output /dev/null --connect-timeout 15 -X POST "$KONG_ADDR"/certificates \
